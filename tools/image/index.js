...
const loopOnPosts =
  async (cb) => {
    ...
    const files = await globby("www/content/blog/*.md", { cwd: rootDir, ignore: ["**/_index.md"] });
    for (const file of files) {
        const content = await fs.readFile(path.resolve(rootDir, file), { encoding: "utf8" });
...
(async function Main () {
  ...
  await loopOnPosts(
    async (post) => {
      ...
      image.print(font, /* x */ 0, /* y */ 116, { 
        text: post.title,
        alignmentX: Jimp.HORIZONTAL_ALIGN_CENTER,
      }, width);

      image.write(
          path.resolve(rootDir, "www/static/images/post-cards", formatName(post.title) + ".png")
      );
    }
  );
